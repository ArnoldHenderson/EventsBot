...
let hostname = 'de1.bruhhost.com'
let serverQueryPort = '27008'
...
module.exports = {
  ...
  run: ({ interaction, client }) => {
    try {
        ...
        const formattedTime = `${hours}:${minutes}`;
        // BEGIN QUERY
        console.log(`[${new Date().toDateString()} @ ${formattedTime}UTC]: Querying '${hostname}:${serverQueryPort}'...`)
        queryGameServerInfo(`${hostname}:${serverQueryPort}`).then(infoResponse => {
           ...
           const queryEmbed = new EmbedBuilder()
            ...
            .setFields(
              { name: 'Game/Version', value: `${infoResponse.game} / ${infoResponse.version}` },
              { name: 'Server Name', value: `${infoResponse.name}` },
              { name: 'Current Map', value: `${infoResponse.map}` },
              { name: 'Players (Bugged)', value: `${infoResponse.players}/${infoResponse.maxPlayers}` },
              { name: 'Server Visibility', value: `${infoResponse.visibility}` },
            )
            .setTimestamp() ;
            console.log(`[${new Date().toDateString()} @ ${formattedTime}UTC]: A2S Query Recieved and Sent!`);
